...
def build_sales_only_train_dataset(name):
    ds = d.read_imputed_onehot_dataset()
    ds = __prepare_sales_only_train_ds(ds)
    d.save_dataset(ds, name)


def build_sales_only_test_dataset(name):
...
def __prepare_sales_only_test_ds(ds, dfrom):
    ...
    ds = pre_u.mean_sales_per_month_per_shop(ds, dfrom)
...
def __prepare_sales_only_train_ds(ds):
    ds['Date'] = p.to_datetime(ds['Date'], format='%d/%m/%Y')
    ds['Day'] = ds['Date'].dt.weekday_name
    ds['Date'] = ds['Date'].apply(lambda x: x.strftime('%Y-%m-%d'))
    ds['Month'] = ds['Date']
    ...
    ds = pre_u.add_min_per_shop(ds)
    ds = pre_u.mean_sales_per_month_per_region(ds)
    ds = pre_u.mean_sales_per_month_per_shop(ds)
...
def create_all_finals_sales_only():
    build_sales_only_train_dataset("final_sales_only_train.csv")
    build_sales_only_test_dataset("final_sales_only_test.csv")


if __name__ == '__main__':
     create_all_finals_sales_only()
     attrs_sal_tr = list(d.read_dataset("final_sales_only_train.csv"))
     pre_u.reorder_datas_cols("final_sales_only_test.csv", attrs_sal_tr, "final_sales_only_test_r.csv")
